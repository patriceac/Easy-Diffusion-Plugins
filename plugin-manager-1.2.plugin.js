    ...
    */
    ...
    async function initPluginTable(plugins) {
        ...
        plugins.forEach(plugin => {
            ...
            newrow.innerHTML = `
                <div>${icon}</div>
                <div><label class="plugin-name">${name}${author}${version}</label>${note}<span class='plugin-source'>Source: <a href="${plugin.url}" target="_blank">${extractFilename(plugin.url)}</a><span></div>
                <div>${localPluginFound ? "<span class='plugin-installed-locally'>Installed locally</span>" : 
                        (isGitHub(plugin.url) ? 
    ...
    async function downloadPlugins(pluginCatalog, plugins, refreshPlugins) {
        ...
        for (const plugin of pluginCatalog) {
            ...
            if (isGitHub(plugin.url) && existingPlugin?.enabled === true && (refreshPlugins || existingPlugin?.sha !== sha || existingPlugin?.code === undefined)) {
                const pluginSource = await getDocument(plugin.url);
                if (pluginSource !== null) {
                    console.log(`Plugin ${plugin.name} downloaded`);
